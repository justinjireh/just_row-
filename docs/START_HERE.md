# Start Here

Use this file when handing the repository to a new collaborator or an LLM.

## If You Need To Understand The Project Fast

Read these first:

1. [../README.md](../README.md)
2. [../HYDROW_CUSTOM_APP_BRIEF.md](../HYDROW_CUSTOM_APP_BRIEF.md)
3. [../HYDROW_IMPLEMENTATION_STATUS_BRIEF.md](../HYDROW_IMPLEMENTATION_STATUS_BRIEF.md)
4. [RELEASE_NOTES_v0.1_PROTOTYPE.md](RELEASE_NOTES_v0.1_PROTOTYPE.md)

## If You Need To Build Or Ship Code

Read:

1. [../CONTRIBUTING.md](../CONTRIBUTING.md)
2. [../LLM_HANDOFF_PROMPT_CODING.md](../LLM_HANDOFF_PROMPT_CODING.md)
3. [../NEXT_STEPS.md](../NEXT_STEPS.md)

## If You Need Real Rowing Metrics

Read:

1. [../device_recon/HYDROW_PACKAGE_RECON.md](../device_recon/HYDROW_PACKAGE_RECON.md)
2. [../LLM_HANDOFF_PROMPT_REVERSE_ENGINEERING.md](../LLM_HANDOFF_PROMPT_REVERSE_ENGINEERING.md)
3. Inspect:
   - `../device_apks/HydrowLauncher.apk`
   - `../device_apks/crew-base.apk`

## If You Need To Improve UX

Read:

1. [../LLM_HANDOFF_PROMPT_PRODUCT_UX.md](../LLM_HANDOFF_PROMPT_PRODUCT_UX.md)
2. [UI_GUIDELINES.md](UI_GUIDELINES.md)
3. [../NEXT_STEPS.md](../NEXT_STEPS.md)

## If You Need A Single General Prompt

Use:

- [../LLM_HANDOFF_PROMPT.md](../LLM_HANDOFF_PROMPT.md)

## If You Need The Full Document Index

Use:

- [README.md](README.md)
